...
export default function QBOtokensUpdateForm(props) {
  const {
    id: idProp,
    qBOtokens: qBOtokensModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = qBOtokensRecord
      ? { ...initialValues, ...qBOtokensRecord }
      : initialValues;
    setAccessToken(cleanValues.accessToken);
    setRealmID(cleanValues.realmID);
    setRefreshToken(cleanValues.refreshToken);
    setNameQBO(cleanValues.nameQBO);
    setErrors({});
  };
  const [qBOtokensRecord, setQBOtokensRecord] =
    React.useState(qBOtokensModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(QBOtokens, idProp)
        : qBOtokensModelProp;
      setQBOtokensRecord(record);
    };
    queryData();
  }, [idProp, qBOtokensModelProp]);
  React.useEffect(resetStateValues, [qBOtokensRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            QBOtokens.copyOf(qBOtokensRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "QBOtokensUpdateForm")}
      {...rest}
    >
      <TextField
        label="Access token"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || qBOtokensModelProp)}
